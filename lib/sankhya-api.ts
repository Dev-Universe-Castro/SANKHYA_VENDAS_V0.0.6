...
async function obterToken(): Promise<string> {
  ...
  try {
    ...
    throw new Error(`Falha na autenticação Sankhya: ${JSON.stringify(errorDetails)}`);
  }
}
...
async function fazerRequisicaoAutenticada(fullUrl: string, method = 'POST', data = {}) {
  ...
  try {
    ...
    if (erro.response && (erro.response.status === 401 || erro.response.status === 403)) {
      console.log("Token expirado ou inválido, limpando cache e solicitando novo token...");
      cachedToken = null;
      throw new Error("Sessão expirada. Tente novamente.");
    }

    throw new Error(`Falha na comunicação com a API Sankhya: ${apiErrorDetails}`);
  }
}
